...
# aquire different range base on different true_label
def index_range(true_label, star_length, gala_length, yso_length):
    if true_label == 0 :
        i_range = range(0, star_length)
    if true_label == 1 :
        i_range = range(star_length, star_length + gala_length)
    if true_label == 2 :
        i_range = range(star_length + gala_length, star_length + gala_length + yso_length)
    return i_range
...
if __name__ == "__main__":
    ...
    cls_pred = None
    cls_true = None
    all_element_in_confusion_matrix = np.array([])
    ...
    for directory in data_list:
        ...
        element_in_confusion_matrix = tracer.test[(cls_true == true_label) &(cls_pred == pred_label)]
        all_element_in_confusion_matrix = np.append(all_element_in_confusion_matrix, element_in_confusion_matrix)
        print ("number of gala to yso: {0}".format(len(element_in_confusion_matrix)))
        # save element_in_confusion_matrix
        np.save("{0}/{1}_true_{2}_pred_{3}.npy".format(directory, keyword, true_[true_label], pred_[pred_label]), 
                element_in_confusion_matrix)
        np.savetxt("{0}/{1}_true_{2}_pred_{3}.txt".format(directory, keyword, true_[true_label], pred_[pred_label]), 
                element_in_confusion_matrix)
    # save all_element_in_confusion_matrix
    np.save("all_true_{0}_pred_{1}.npy".format(true_[true_label], pred_[pred_label]), all_element_in_confusion_matrix)
    np.savetxt("all_true_{0}_pred_{1}.txt".format(true_[true_label], pred_[pred_label]), all_element_in_confusion_matrix)
    # plot the result
    i_range = index_range(true_label, star_length, gala_length, yso_length)
    numbers, index = np.histogram(all_element_in_confusion_matrix, i_range)
    result_plt = plt.figure("histogram of true: {0}, pred: {1}".format(true_[true_label], pred_[pred_label]))
    plt.plot(index[:-1], numbers)
